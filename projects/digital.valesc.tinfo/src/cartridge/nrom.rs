...
mod tests {
    ...
    fn test_write_protection() {
        ...
        unsafe {
            ...
            assert!(nrom_cartridge.write(NROM_FIRST_ROM_BANK_ADDRESS, 0).is_err());
            assert!(nrom_cartridge.write(NROM_SECOND_ROM_BANK_ADDRESS, 0).is_err());
        }
    }
    ...
    fn test_read_below_prg_protection() {
        ...
        unsafe {
            assert!(nrom_cartridge.read(INVALID_NROM_ADDRESS).is_err())
        }
    }
    ...
    #[test]
    fn test_read_on_16k() {
        let mut nrom_cartridge = Nrom::new(false, MockRom {});
